...
class UI_Function(MainGUI):
    ...
    def update(self):
        ...
        img = ps.putBText(img,text,text_offset_x=20,text_offset_y=80,vspace=20,hspace=10, font_scale=1,background_RGB=(10,20,222),text_RGB=(255,255,255))
        text = str(time.strftime("%H:%M %p"))
        img = ps.putBText(img,text,text_offset_x=self.image.shape[1]-180,text_offset_y=30,vspace=20,hspace=10, font_scale=1,background_RGB=(228,20,222),text_RGB=(255,255,255))
        text  =  f"Brightness: {self.brightness_value_now}"
        img = ps.putBText(img,text,text_offset_x=20,text_offset_y=425,vspace=20,hspace=10, font_scale=1,background_RGB=(20,210,4),text_RGB=(255,255,255))
        text  =  f'Blur: {self.blur_value_now}: '
        img = ps.putBText(img,text,text_offset_x=self.image.shape[1]-160,text_offset_y=425,vspace=20,hspace=10, font_scale=1,background_RGB=(210,20,4),text_RGB=(255,255,255))
    ...
    def savePhoto(self):
        ...
        self.ui.filename = 'Snapshot '+str(time.strftime("%Y-%b-%d at %H.%M.%S %p"))+'.png'
        path = os.path.join('images/saving pics',self.ui.filename)
        cv2.imwrite(path,self.tmp)
        UI_Function.saving_confirm(self,path)
        print('Image saved as:',self.ui.filename)
    ...
    def saving_confirm(self, filename,event = None):
        """
        This function will generate the Dialog to inform user to complete the process before running.
        """
        msg = QMessageBox()
        msg.setWindowTitle("Photos information")
        msg.setText("Image saving sucessfull at:\n"+f"{filename}")
